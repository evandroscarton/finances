...
def listar_com_filtros(cur, usuario_id, filtros, contas_visiveis_ids=None, cartoes_visiveis_ids=None):
    """
    Filtros suportados: conta_id, cartao_id, categoria_id, ultimo_lancamento_inicio,
    ultimo_lancamento_fim — todos opcionais, montados dinamicamente com
    parâmetros (%s), nunca concatenação de string.

    ultimo_lancamento_inicio/fim filtram pelo intervalo de `ultimo_lancamento_em`
    (data em que este modelo foi efetivamente lançado por último). Registros
    que nunca foram lançados (ultimo_lancamento_em IS NULL) são excluídos do
    resultado quando qualquer um dos dois limites é informado, pois não há
    data para comparar.
    ...
    if filtros.get("ultimo_lancamento_inicio"):
        sql += " AND lp.ultimo_lancamento_em >= %s"
        params.append(filtros["ultimo_lancamento_inicio"])
    if filtros.get("ultimo_lancamento_fim"):
        sql += " AND lp.ultimo_lancamento_em <= %s"
        params.append(filtros["ultimo_lancamento_fim"])
