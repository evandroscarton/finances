...
def listar_contas(cur, usuario_id, contas_visiveis_ids=None):
    """
    contas_visiveis_ids: None = sem restrição (vê todas as próprias contas).
    Lista de IDs = restringe às contas permitidas (regra de permissoes_service).
    """
    sql = "SELECT * FROM contas WHERE usuario_id = %s"
    params = [usuario_id]
