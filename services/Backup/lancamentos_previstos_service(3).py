...
a partir de dia_mes + mes_ano. O registro previsto NUNCA é apagado ao lançar
— o usuário pode trocar o mes_ano e lançar de novo em meses futuros. A cada
lançamento efetivo, gravamos `ultimo_lancamento_em` com a data atual, apenas
para auditoria/controle visual do usuário.
"""
import calendar
...
def lancar_previsto(previsto_id, usuario_id):
    ...
    data_lancamento = dia_mes + mes_ano do previsto. Não altera o previsto,
    exceto por gravar `ultimo_lancamento_em` = data atual (auditoria/controle
    visual de quando esse modelo foi usado por último).
    """
    ...
    with get_cursor(commit=True) as cur:
        repo.atualizar_ultimo_lancamento(cur, previsto_id, usuario_id, date.today())

    return lancamento

def atualizar_mes_ano_em_massa(usuario_id, mes_ano_str):
    """
    Valida o mês/ano recebido do formulário (input type="month", "YYYY-MM")
    e aplica a todos os previstos do usuário em uma única transação.
    """
    try:
        ano, mes = mes_ano_str.split("-")
        mes_ano = date(int(ano), int(mes), 1)
    ...
    with get_cursor(commit=True) as cur:
        linhas = repo.atualizar_mes_ano_em_massa(cur, usuario_id, mes_ano)
    return linhas
