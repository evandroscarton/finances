...
- linhas_pdf: cada linha do PDF com status "encontrado" ou "nao_encontrado".
  Quando encontrado, inclui "descricao_sistema" (descrição do lançamento
  no sistema) para exibição na tela, já que a descrição do PDF costuma ser
  menos legível que a descrição cadastrada pelo próprio usuário.
        ...
        if casamento:
            ...
            resultado_linhas_pdf.append({
                **linha,
                ...
                "descricao_sistema": casamento["descricao"],
            })
        else:
            resultado_linhas_pdf.append({
                **linha,
