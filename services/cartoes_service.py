...
def calcular_fatura_atual(cartao_id, usuario_id):
    """Soma dos lançamentos pendentes do cartão (fatura em aberto), sem filtro de período."""
    with get_cursor() as cur:
        cur.execute(
            """
...
def listar_lancamentos_pendentes(cartao_id, usuario_id, data_inicio=None, data_fim=None):
    """
    Lista lançamentos pendentes do cartão, opcionalmente restritos a um
    intervalo de DATA DE COMPENSAÇÃO (não confundir com data_lancamento).
    Usado na tela de Fatura para permitir pagamento parcial por período
    (ex.: pagar apenas o que compensa até o fechamento do mês).
    """
    sql = """
    ...
    sql += " ORDER BY l.data_compensacao ASC, l.data_lancamento DESC"
